...
- Outside the documented Release 30 bindings, `document.thisItem`, `parent.thisItem`, and frame/window traversal remain compatibility-sensitive. Release 30 documents `document.thisItem` for Form/Grid/Field handlers and `parent.thisItem` for relationship-grid handlers only.
